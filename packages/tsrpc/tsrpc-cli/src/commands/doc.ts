...
import { ProtoGeneratorOptions } from '@ntsrpc/tsbuffer-proto-generator'
    ...
    if (options.config) {
        ...
        for (const conf of options.config.proto) {
            options.config.verbose &&
                console.log(`Start to generate ${conf.output}...`)
            if (!conf.docDir) {
                continue
            }
            await generate(
...
async function generate(
    ...
    // Generate proto
    const { newProto } = await ProtoUtil.generateServiceProto({
        protocolDir: ptlDir,
        ignore: ignore,
        verbose: verbose,
        checkOptimize: false,
...
async function generateTSAPI(proto: ServiceProto, outputDir: string) {
    // Generate OpenAPI
    const tsrpcAPI = await ApiDocUtil.toTSAPI(proto)
...
async function generateMarkdown(api: TSAPI, outputDir: string) {
    const md = ApiDocUtil.toMarkdown(api)
    await fs.ensureDir(outputDir)
    const outputPath = path.resolve(outputDir, 'tsapi.md')
