...
export async function cmdSync(options: CmdSyncOptions) {
    if (options.config) {
        ...
        CliUtil.doing(
            `${i18n.copy} '${path.resolve(options.from)}' -> '${path.resolve(
                options.to,
            )}'`,
        )
        await copyDirReadonly(
            ...
            true,
            true,
            options.verbose ? console : undefined,
        )
        CliUtil.done(true)
...
export async function syncByConfig(
    syncConfig: NonNullable<TsrpcConfig['sync']>,
...
export async function copyDirReadonly(
    src: string,
    dst: string,
    clean: boolean,
    readonly: boolean,
...
export async function setReadonlyRecursive(dst: string, logger?: Logger) {
    ...
    const items = await new Promise<string[]>((rs, rj) => {
