...
export async function cmdDev(options: CmdDevOptions) {
    ...
    const cmdStart =
        'node ' +
        devServerArgs.map((v) => (/\s/.test(v) ? `'${v}'` : v)).join(' ')
    const watchFiles = conf.dev?.watch ?? 'src'
    const protoErr: { [protoPath: string]: string } = {}
    ...
    const firstSyncedIndices: number[] = []
    if (conf.sync) {
        const copyConfs = conf.sync.filter((v) => v.type === 'copy')
        for (let i = 0; i < copyConfs.length; ++i) {
            const conf = copyConfs[i]
            const lstat = await fse.lstat(conf.to)
            if (lstat.isSymbolicLink() || lstat.isFile()) {
                await fse.remove(conf.to)
                await copyDirReadonly(
                    conf.from,
                    conf.to,
                    !!conf.clean,
                    conf.readonly ?? true,
                    options.config.verbose ? console : undefined,
                )
                console.log(
                    chalk.green(
                        `✔ ${i18n.copy} '${conf.from}' -> '${conf.to}'`,
                    ),
                )
    ...
    if (autoProto && conf.proto) {
        for (const confItem of conf.proto) {
            ...
            // old
            const old = await ProtoUtil.loadOldProtoByConfigItem(
                confItem,
                options.config.verbose,
            )

            const onAutoProtoTrigger = async () => {
                const newProto = await ProtoUtil.genProtoByConfigItem(
                    confItem,
                    old,
                    options.config.verbose,
                    options.config.checkOptimizableProto,
                    true,
                ).catch((e) => {
                    // 插入错误记录
            ...
            delayWatch({
                ...
                ignore: [
                    confItem.output,
                    ...(confItem.compatible ? [confItem.compatible] : []),
                    ...(confItem.ignore ?? []),
                ],
                onChange: async (eventName, filepath, stats) => {
                    ...
                    if (
                        path
                            .relative(confItem.ptlDir, filepath)
                            .startsWith('..')
                    ) {
                        return
                    }

                    const type = match[1] as 'Ptl' | 'Msg'
                    const basename = match[2]

                    // Auto fill new Ptl or Msg
                    if (
                        (conf.dev?.autoFillNewPtl ?? true) &&
                        eventName === 'add'
                    ) {
                        await fillNewPtlOrMsg(filepath, confItem, {
                            type,
                            basename,
                        })
                    }
                    ...
                    if (autoApi && confItem.apiDir && type === 'Ptl') {
                        ...
                        const emptyApiContent = (
                            confItem.apiTemplate ?? CodeTemplate.defaultApi
                        )(
                            basename,
                            path.dirname(apiFilePath),
                            path.dirname(filepath),
                        )
                        ...
                        if (eventName === 'unlink') {
                            // File not exists
                            if (
                                await fse.access(apiFilePath).catch(() => true)
                            ) {
                                return
                            }

                            // Compare, only remove when it is unchanged
                            const apiContent = await fse
                                .readFile(apiFilePath, 'utf-8')
                                .catch(() => undefined)
                            if (
                                apiContent?.length === 0 ||
                                apiContent === emptyApiContent
                            ) {
                ...
                onTrigger: async (
                    ...
                    if (!isInited) {
                        await copyDirReadonly(
                            ...
                            confItem.to,
                            !!confItem.clean,
                            confItem.readonly ?? true,
                            options.config.verbose ? console : undefined,
                        )
                        console.log(
                            chalk.green(
                                `✔ ${i18n.copy} '${confItem.from}' -> '${confItem.to}'`,
                            ),
                        )
                        isInited = true
                    }
                    // 后续改为增量
                        ...
                        const dstPath = path.resolve(
                            confItem.to,
                            path.relative(confItem.from, filepath),
                        )
                        // 删除
    ...
    const startDevServer = async () => {
        const restartTimes = devServerRestartTimes
        ...
        for (const protoPath in protoErr) {
        ...
        console.log(
            `${chalk.green(i18n.startDevServer)} ${chalk.cyan(cmdStart)}\n`,
        )
...
export async function fillNewPtlOrMsg(
    filepath: string,
    confItem: NonNullable<TsrpcConfig['proto']>[number],
    parsed?: {
        type: 'Ptl' | 'Msg'
        basename: string
    },
    ...
    if (!parsed) {
        const match = path.basename(filepath).match(/^(Ptl|Msg)(\w+)\.ts$/)
        if (!match) {
            return false
        }
        parsed = {
    ...
    if (parsed.type === 'Ptl') {
        await fse.writeFile(
            filepath,
            getPtlTemplate(confItem.ptlTemplate)(
                parsed.basename,
                filepath,
                confItem.ptlDir,
            ),
        ...
        await fse.writeFile(
            filepath,
            getMsgTemplate(confItem.msgTemplate)(
                parsed.basename,
                filepath,
                confItem.ptlDir,
            ),
...
export async function fillAllPtlAndMsgs(
    confItem: NonNullable<TsrpcConfig['proto']>[number],
    autoApi = false,
) {
    const files = await new Promise<string[]>((rs, rj) => {
        glob(
            path.resolve(path.resolve(confItem.ptlDir, '**/{Ptl,Msg}?*.ts')),
            (err, matches) => {
                if (err) {
                    rj(err)
                } else {
    ...
    for (const file of files) {
        ...
        const match = path.basename(file).match(/^(Ptl|Msg)(\w+)\.ts$/)!
        const type = match[1]
        const basename = match[2]
        ...
        if (autoApi && type === 'Ptl' && confItem.apiDir) {
            const apiFilePath = path.join(
                ...
                path.relative(
                    confItem.ptlDir,
                    path.join(path.dirname(file), `Api${basename}.ts`),
                ),
            )
            await genNewApiFile(
                basename,
...
function getPtlTemplate(
    ptlTemplate: NonNullable<TsrpcConfig['proto']>[0]['ptlTemplate'],
): PtlTemplate {
    if (!ptlTemplate) {
    ...
    if (ptlTemplate.baseFile) {
        return CodeTemplate.getExtendedPtl(
            ptlTemplate.baseFile,
            ptlTemplate.baseReq,
            ptlTemplate.baseRes,
            ptlTemplate.baseConf,
        )
    }
...
function getMsgTemplate(
    msgTemplate: NonNullable<TsrpcConfig['proto']>[0]['msgTemplate'],
): MsgTemplate {
    if (!msgTemplate) {
    ...
    if (msgTemplate.baseFile) {
        return CodeTemplate.getExtendedMsg(
            msgTemplate.baseFile,
            msgTemplate.baseMsg,
            msgTemplate.baseConf,
        )
    }
