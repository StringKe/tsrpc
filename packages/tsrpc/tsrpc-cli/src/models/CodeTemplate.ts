...
export default async function (call: ApiCall<Req${apiBaseName}, Res${apiBaseName}>) {
     ...
     * 自公共基类继承的 Ptl
     */
    static getExtendedPtl(
        baseFile = 'src/shared/protocols/base.ts',
        baseReq = 'BaseRequest',
        ...
        return (ptlBaseName, ptlPath, ptlDir) => {
            const importPath = path
                .relative(
                    path.dirname(ptlPath),
                    path.resolve(baseFile.replace(/\.ts$/, '')),
                )
                .replace(/\\/g, '/')
            return `import { ${baseReq}, ${baseRes}, ${baseConf} } from "${
...
export const conf: ${baseConf} = {
     ...
     * 自公共基类继承的 Msg
     */
    static getExtendedMsg(
        baseFile = 'src/shared/protocols/base.ts',
        baseMsg = 'BaseMessage',
        baseConf = 'BaseConf',
    ): MsgTemplate {
        return (msgBaseName, msgPath, msgDir) =>
            `import { ${baseMsg}, ${baseConf} } from "./${path
                .relative(
                    path.dirname(msgPath),
                    path.resolve(baseFile.replace(/\.ts$/, '')),
                )
                .replace(/\\/g, '/')}";
