...
export class ApiDocUtil {
    ...
    static init(proto: ServiceProto<any>) {
        // Custom types
        proto.types['?mongodb/ObjectId'] =
            proto.types['?mongodb/ObjectID'] =
            proto.types['?bson/ObjectId'] =
            proto.types['?bson/ObjectID'] =
                {
        ...
        const generator = new TSBufferValidator(proto.types)
    ...
    static toOpenAPI(proto: ServiceProto<any>): OpenAPIV3.Document {
        ...
        const pathObj: OpenAPIV3.PathsObject = Object.fromEntries(
            apiSvcs.map((v) => {
                ...
                const pathValue: OpenAPIV3.PathItemObject = {
                    post: {
                        tags: tags,
                        description: (
                            schemas[v.reqSchemaId] as OpenAPIV3.SchemaObject
                        )?.description,
                        operationId: v.name,
                        requestBody: {
        ...
        const output: OpenAPIV3.Document = {
            openapi: '3.0.0',
            info: {
                        ...
                        content: {
                            'application/json': {
                                schema: {
                                    ...
                                    title: 'API 错误',
                                    description:
                                        '业务错误（ApiError）返回 HTTP 状态码 200，其它错误返回 HTTP 状态码 500',
                                    properties: {
                                        isSucc: {
                                            type: 'boolean',
                                            enum: [false],
    ...
    static toSchemaObject(schema: TSBufferSchema): OpenAPIV3.SchemaObject {
        ...
        switch (schema.type) {
            ...
            case SchemaType.Number:
                if (
                    schema.scalarType &&
                    schema.scalarType.indexOf('int') > -1
                ) {
                    output.type = 'integer'
                } else {
                    output.type = 'number'
            ...
            case SchemaType.Tuple:
                output = {
                    ...output,
                    type: 'array',
                    prefixItems: schema.elementTypes.map((v) =>
                        this.toSchemaObject(v),
            ...
            case SchemaType.Enum:
                if (schema.members.every((v) => typeof v.value === 'string')) {
                    ...
                    schema.members.every((v) => typeof v.value === 'number')
                ) {
                    output.type = 'number'
                } else {
            ...
            case SchemaType.Interface: {
                    ...
                    output.properties = Object.fromEntries(
                        schemaProperties.map((property) => {
                            ...
                            if (property.type.type === SchemaType.Union) {
                                const members = property.type.members.filter(
                                    (v) =>
                                        !(
                                            v.type.type ===
                                                SchemaType.Literal &&
                            ...
                            return [
                                property.name,
                                this.toSchemaObject(property.type),
                            ]
                        }),
                    )

                    output.required = schemaProperties
                        .filter((v) => !v.optional)
                        .map((v) => v.name)
                ...
                if (schema.indexSignature) {
                    output.additionalProperties = this.toSchemaObject(
            ...
            case SchemaType.IndexedAccess: {
                const parsed = this.protoHelper.parseReference(schema)
                output = {
                    ...output,
            ...
            case SchemaType.Union: {
                const members = schema.members.filter((v) => {
                    const type = v.type
                    return !(
                        type.type === SchemaType.Literal && type.literal == null
                    )
                })
                ...
                // >= 2 members
                    ...
                    const flats = members.map((v) =>
                        this.protoHelper.isInterface(v.type)
                            ? this.protoHelper.getFlatInterfaceSchema(v.type)
                            : null,
                    )
                    if (flats.every((v) => !!v)) {
                        // Every member has a same literal property
                        flats[0]?.properties.some((disProp) => {
                            const literalTypes = flats.map((f) => {
                                const prop = f?.properties.find(
                                    (v) => v.name === disProp.name,
                                )
                                if (
                                    !prop ||
                                    prop.type.type !== SchemaType.Literal ||
                                    prop.optional
                                ) {
                                    return null
                                }
            ...
            case SchemaType.Intersection:
                output.allOf = schema.members.map((v) =>
                    this.toSchemaObject(v.type),
                )
                break
            ...
            case SchemaType.Overwrite: {
                const parsed = this._parseMappedType(schema)
    ...
    static async toCode(
            ...
            code +=
                ...
                this._toCodeData.refs
                    .map((v) => {
                        ...
                        if (proto[v].comment) {
                            refCode =
                                this._toCodeComment(proto[v].comment!) +
                                '\n' +
                                refCode
                        }
                        return refCode
        ...
        // Format
        const format = await processString('a.ts', code, {} as any)
        return format.dest
    }
    ...
    static async toTSAPI(proto: ServiceProto): Promise<TSAPI> {
        ...
        for (const api of apiSvcs) {
            ...
            const commentArr = proto.types[api.reqSchemaId].comment
                ?.trim()
                ?.split('\n')
            const title = commentArr?.[0].trim()
            const remark = commentArr?.slice(1).join('\n')
...
`
        md += '## 目录\n\n'
        md +=
            this._treeToToc(this._toApiTree(api.apis, '/', 10), 1) + '\n---\n\n'
    ...
    private static _generateCircularRefs(schema: TSBufferSchema) {
        switch (schema.type) {
            case SchemaType.Array:
                ...
                return
            case SchemaType.Tuple:
                schema.elementTypes.forEach((v) => {
                    this._generateCircularRefs(v)
                })
                return
            case SchemaType.Interface:
                schema.extends?.forEach((v) => {
                    this._generateCircularRefs(v.type)
                })
                schema.properties?.forEach((v) => {
                    this._generateCircularRefs(v.type)
                })
                schema.indexSignature &&
                    this._generateCircularRefs(schema.indexSignature.type)
                return
            case SchemaType.IndexedAccess:
            ...
            case SchemaType.Reference:
                ...
                this._generateCircularRefs(
                    this._genCircularData.proto[schema.target],
                )
                this._genCircularData?.refStack.pop()
                return
            case SchemaType.Union:
            case SchemaType.Intersection:
                schema.members.forEach((v) => {
                    this._generateCircularRefs(v.type)
                })
                return
            case SchemaType.NonNullable:
    ...
    private static _toCode(
        schema: TSBufferSchema,
        options?: { isRoot?: boolean; schemaId?: string },
    ): string {
        switch (schema.type) {
            case SchemaType.Boolean:
                return 'boolean'
            ...
            case SchemaType.String:
                return 'string'
            case SchemaType.Array:
                const elemType = this.protoHelper.isTypeReference(
                    schema.elementType,
                )
                    ? this.protoHelper.parseReference(schema.elementType)
                    : schema.elementType
                const code = this._toCode(schema.elementType)
                return elemType.type === SchemaType.Union ||
                    elemType.type === SchemaType.Intersection
                    ? `(${code})[]`
                    : `${code}[]`
            case SchemaType.Tuple:
            ...
            case SchemaType.Interface: {
                const flat = this.protoHelper.getFlatInterfaceSchema(schema)
                const props: string[] = []
                for (const prop of flat.properties) {
                    ...
                    // 字段无 Comment，但是是引用
                        ...
                        // 引用有 Comment
                        const parsedSchema = this.protoHelper.parseReference(
                            prop.type,
                        )
                        if (parsedSchema.comment) {
                            propStr += `${this._toCodeComment(
                                parsedSchema.comment,
                            )}\n`
                        }
                    }

                    propStr += `${
                        /^[a-z_]/i.test(prop.name)
                            ? prop.name
                            : `'${prop.name}'`
                    }${prop.optional ? '?' : ''}: ${this._toCode(prop.type)}`
                    props.push(propStr)
                }
                if (flat.indexSignature) {
                    props.push(
                        `[key: ${flat.indexSignature.keyType.toLowerCase()}]: ${this._toCode(
            ...
            case SchemaType.Union:
                return schema.members
                    .map((v) => {
                        const parsed = this.protoHelper.isTypeReference(v.type)
                            ? this.protoHelper.parseReference(v.type)
                            : v.type
                        const code = this._toCode(v.type)
                        return parsed.type === SchemaType.Intersection
                            ? `(${code})`
                            : code
                    })
                ...
                return schema.members
                    .map((v) => {
                        const parsed = this.protoHelper.isTypeReference(v.type)
                            ...
                            : v.type
                        const code = this._toCode(v.type)
                        return parsed.type === SchemaType.Union
                            ? `(${code})`
                            : code
                    })
            ...
            case SchemaType.Custom: {
                const schemaId = options?.schemaId?.toLowerCase()
                if (
                    schemaId === '?mongodb/objectid' ||
                    schemaId === '?json/objectid'
                ) {
    ...
    private static _parseMappedType(
        schema:
            | PickTypeSchema
            | OmitTypeSchema
            | OverwriteTypeSchema
            | PartialTypeSchema,
    ): FlatInterfaceTypeSchema | UnionTypeSchema | IntersectionTypeSchema {
        if (this.protoHelper.isInterface(schema)) {
            return this.protoHelper.getFlatInterfaceSchema(schema)
        } else {
            return this.protoHelper.parseMappedType(schema) as
                | UnionTypeSchema
                | IntersectionTypeSchema
        }
    }
    ...
    private static _treeToToc(tree: ApiTreeNode[], currentDepth: number) {
        ...
        for (const node of tree) {
            // 写个标题
            if (node.type === 'api') {
                output += `${' '.repeat((currentDepth - 1) * 4)}- [${
                    node.name
                }](#${node.api.path})\n`
    ...
    private static _treeToMarkdown(tree: ApiTreeNode[], currentDepth: number) {
        ...
        for (const node of tree) {
            // 写个标题
            let part = `${'#'.repeat(currentDepth + 1)} ${node.name}${
                node.type === 'api' ? ` <a id='${node.api.path}'></a>` : ''
            }\n\n`
            ...
            // api
                ...
                if (
                    node.api.conf &&
                    (typeof node.api.conf !== 'object' ||
                        Object.keys(node.api.conf).length > 0)
                ) {
                    part +=
                        '**配置**\n```ts\n' +
    ...
    private static _toApiTree(
        ...
        const typedApis = apis.map((v) => {
            const arrPrefix = prefix.split('/')
            const isLastFolderDepth = arrPrefix.length === maxDepth
            const groupArr = v.path.slice(prefix.length).split('/')
            const groupName = isLastFolderDepth
                ? groupArr.slice(0, -1).join('/')
                : groupArr[0]
        ...
        // 计算这一级的 API 节点
        const apiNodes: ApiTreeApiNode[] = typedApis
            .filter((v) => v.type === 'api')
            .map((v) => {
                return {
                    type: 'api',
