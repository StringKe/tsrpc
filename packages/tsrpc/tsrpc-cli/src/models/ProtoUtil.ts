...
} from '@ntsrpc/tsbuffer-proto-generator'
import { TSBufferProto } from '@ntsrpc/tsbuffer-schema'
import { Logger, ServiceDef, ServiceProto } from '@ntsrpc/tsrpc-proto'
...
export class ProtoUtil {
    static async loadServiceProto(
        filepath: string,
        logger?: Logger,
        useRegExpWhenTsError = true,
    ): Promise<ServiceProto<any> | undefined> {
        if (await fs.access(filepath).catch((e) => true)) {
            logger?.error(
                formatStr(i18n.fileNotExists, { file: path.resolve(filepath) }),
            )
            return undefined
        }
        ...
        if (filepath.endsWith('.ts')) {
            ...
            if (useRegExpWhenTsError) {
                ...
                if (fileContent) {
                    ...
                    const match = fileContent.match(
                        /export const serviceProto: ServiceProto<ServiceType> = (\{[\s\S]+\});/,
                    )
                    if (match) {
                        try {
            ...
            // 解析失败
            logger?.error(
                formatStr(i18n.protoParsedError, {
            ...
            logger?.warn(
                formatStr(i18n.invalidProtoExt, {
                    file: path.resolve(filepath),
                }),
            )
            return undefined
    ...
    static async parseProtoAndSchema(
    ...
    static async generateServiceProto(options: {
        ...
        // 标准化路径（抹平系统差异）
        const protocolDir = options.protocolDir
            .replace(/\\/g, '/')
            .replace(/\/+$/, '')
        // 只能填写文件夹 不支持通配符
        ...
        try {
            var typeProto = await new TSBufferProtoGenerator({
                ...
                filter: (info) => {
                    const infoPath = info.path.replace(/\\/g, '/')
                    const match = infoPath.match(EXP_DIR_TYPE_NAME)
                    ...
                    if (match[2] === 'Ptl') {
                        return (
                            info.name === 'Req' + match[3] ||
                            info.name === 'Res' + match[3]
                        )
                    } else {
        ...
        for (const filepath of fileList) {
            const match = filepath.match(EXP_DIR_TYPE_NAME)!
            ...
            if (match[2] === 'Ptl') {
                const req = typePath + '/Req' + match[3]
                const res = typePath + '/Res' + match[3]
            ...
            // Msg 检测Msg类型在
                ...
                const msg = typePath + '/Msg' + match[3]
                if (typeProto[msg]) {
                    services.push({
                        id: services.length,
                        name: (match[1] || '') + match[3],
        ...
        const encodeIds = EncodeIdUtil.genEncodeIds(
            services.map((v) => v.type + v.name),
            oldProto?.services.map((v) => ({
                key: v.type + v.name,
                id: v.id,
            })),
        )
        for (const item of encodeIds) {
            services.find(
                (v) =>
                    item.key.startsWith(v.type) &&
                    v.name === item.key.substr(v.type.length),
            )!.id = item.id
        }
        ...
        if (
            oldProto &&
            JSON.stringify({
                types: oldProto.types,
                services: oldProto.services,
            }) !== JSON.stringify({ types: typeProto, services: services })
        ) {
            version = (oldProto.version || 0) + 1
        }

        // 创建新 Proto
        const newProto: ServiceProto = {
            version: version,
            services: services,
            types: typeProto,
        }
        ...
        if (
            options.checkOptimize &&
            canOptimizeByNew &&
            options.oldProto?.path
        ) {
            console.warn(
                chalk.yellow(
                    i18n.canOptimizeByNew(path.resolve(options.oldProto.path)) +
                        '\n',
                ),
            )
        }
    ...
    static async outputProto(
        ...
        if (options.noEmitWhenNoChange) {
            // TS 报错也算需要重新生成
            const oldProto = await this.loadServiceProto(
                options.newProtoPath,
                undefined,
                false,
            )
            if (
                oldProto &&
                JSON.stringify(oldProto) === JSON.stringify(options.proto)
            ) {
                return { emited: false }
            }
        }
        ...
        if (options.newProtoPath.endsWith('.ts')) {
            const imports: {
                [path: string]: { srcName: string; asName?: string }[]
            } = {}
            const apis: {
                name: string
                importPath: string
            ...
            const msgs: { name: string; importPath: string; msg: string }[] = []
            ...
            const addImport = (path: string, srcNames: string[]): string[] => {
                const asNames = srcNames.map((v) => getAsName(v))
                imports[path] = srcNames.map((v, i) => ({
                    srcName: v,
                    asName:
                        asNames[i] && asNames[i] !== v ? asNames[i] : undefined,
                }))
            ...
            for (const svc of options.proto.services) {
                ...
                if (svc.type === 'api') {
                    ...
                    const op = addImport(importPath, ['Msg' + lastName])
                    msgs.push({
                        name: svc.name,
                        importPath: importPath,
                        msg: op[0],
            ...
            res: ${v.res}
                ...
                .join(',\n')
            const msgStr = msgs
                .map((v) => `        ${JSON.stringify(v.name)}: ${v.msg}`)
                .join(',\n')
...
export const serviceProto: ServiceProto<ServiceType> = ${JSON.stringify(
                options.proto,
                null,
                4,
            )};
`.trim()
        ...
        // JSON
            ...
            await fs.writeFile(
                options.newProtoPath,
                options.ugly
                    ? JSON.stringify(options.proto)
                    : JSON.stringify(options.proto, null, 2),
            )
        }
        logger?.log(
            chalk.green(
                formatStr(i18n.protoSucc, {
                    output: path.resolve(options.newProtoPath),
                }),
            ),
        )
    ...
    static async loadOldProtoByConfigItem(
        confItem: Pick<
            NonNullable<TsrpcConfig['proto']>[0],
            'compatible' | 'output'
        >,
    ...
    > {
        // old
        const oldProtoPath = confItem.compatible ?? confItem.output
        let oldProto: ServiceProto<any> | undefined
        if (oldProtoPath) {
            oldProto = await ProtoUtil.loadServiceProto(
                oldProtoPath,
                verbose ? console : undefined,
            )
        }
        verbose &&
            console.log(
                `oldProtoPath: ${oldProtoPath}, hasOldProto=${!!oldProto}`,
            )
    ...
    static async genProtoByConfigItem(
        confItem: Pick<
            NonNullable<TsrpcConfig['proto']>[0],
            'ptlDir' | 'ignore' | 'output' | 'resolveModule'
        >,
        old:
            | {
                  proto: ServiceProto<any>
            ...
            | undefined,
        verbose: boolean | undefined,
        checkOptimize: boolean | undefined,
        noEmitWhenNoChange?: boolean,
        keepComment?: boolean,
    ) {
        // new
        try {
            ...
            verbose &&
                console.log(
                    `Proto generated succ, start to write output file...`,
                )
        } catch (e: any) {
