...
> extends BaseClient<ServiceType> {
    readonly type = 'SHORT'
    override readonly options!: Readonly<BaseHttpClientOptions>
    ...
    constructor(
        ...
        http: IHttpProxy,
        options?: Partial<BaseHttpClientOptions>,
    ) {
        super(proto, {
    ...
    protected async _sendData(
        ...
        const promise = (async (): Promise<
            | { err: TsrpcError | undefined; res?: undefined }
            | { res: string | Uint8Array; err?: undefined }
        > => {
            // Send to this URL
            ...
            const url =
                typeof data === 'string'
                    ? this._jsonServer + service.name + urlSearch
                    : this.options.server

            // Do Send
            const { promise: fetchPromise, abort } = this._http.fetch({
                url: url,
                data: data,
                method: 'POST',
...
export const defaultBaseHttpClientOptions: BaseHttpClientOptions = {
    ...defaultBaseClientOptions,
    server: 'http://localhost:3000', // logger: new TerminalColorLogger(),
    jsonPrune: true,
}
