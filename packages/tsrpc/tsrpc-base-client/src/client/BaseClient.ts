...
export abstract class BaseClient<ServiceType extends BaseServiceType> {
    ...
    readonly flows = {
        ...
        postConnectFlow: new Flow<Record<string, never>>(),
    ...
    protected _onRecvBuf: (
        buf: Uint8Array,
        pendingApiItem?: PendingApiItem,
    ) => Promise<void> = this._onRecvData
     ...
     */
    async callApi<T extends string & keyof ServiceType['api']>(
        apiName: T,
        req: ServiceType['api'][T]['req'],
        options: TransportOptions = {},
    ): Promise<ApiReturn<ServiceType['api'][T]['res']>> {
        // Add pendings
        const sn = this._apiSnCounter.getNext()
        const pendingItem: PendingApiItem = {
            sn: sn,
        ...
        const promise = new Promise<ApiReturn<ServiceType['api'][T]['res']>>(
            async (rs) => {
                // Pre Call Flow
                const pre = await this.flows.preCallApiFlow.exec(
                    {
                        apiName: apiName,
                        req: req,
                        options: options,
                ...
                if (ret.isSucc) {
                    this.options.logApi &&
                        this.logger?.log(
                            `[ApiRes] #${pendingItem.sn} ${apiName}`,
                            ret.res,
                        )
                } else {
                    this.options.logApi &&
                        this.logger?.[
                            ret.err.type === TsrpcError.Type.ApiError
                                ? 'log'
                                : 'error'
                        ](`[ApiErr] #${pendingItem.sn} ${apiName}`, ret.err)
                }

                // Pre Return Flow
                const preReturn = await this.flows.preApiReturnFlow.exec(
                    {
                        ...pre,
                        return: ret,
                    },
        ...
        promise.catch().then(() => {
            remove(this._pendingApis, (value) => value.sn === pendingItem.sn)
        })
     ...
     */
    sendMsg<T extends string & keyof ServiceType['msg']>(
        msgName: T,
        msg: ServiceType['msg'][T],
        options: TransportOptions = {},
    ): Promise<{ isSucc: true } | { isSucc: false; err: TsrpcError }> {
        const promise = new Promise<
            { isSucc: true } | { isSucc: false; err: TsrpcError }
        >(async (rs) => {
            // Pre Flow
            ...
            // Encode
            const opEncode = TransportDataUtil.encodeClientMsg(
                this.tsbuffer,
                service,
                msg,
                this.dataType,
            ...
            // Send Buf...
            const promiseSend = this.sendData(
                opEncode.output,
                options,
                service.id,
            )
    ...
    listenMsg<T extends keyof ServiceType['msg']>(
        msgName: T | RegExp,
        handler: ClientMsgHandler<ServiceType, T>,
    ): ClientMsgHandler<ServiceType, T> {
    ...
    unlistenMsg<T extends keyof ServiceType['msg']>(
        msgName: T | RegExp, // eslint-disable-next-line @typescript-eslint/ban-types
        handler: Function,
    ) {
        if (msgName instanceof RegExp) {
            Object.keys(this.serviceMap.msgName2Service)
    ...
    abort(sn: number): void {
        // Find
        const index = this._pendingApis.findIndex((v) => v.sn === sn)
        if (index === -1) {
            return
        }
        const pendingItem = this._pendingApis[index]
        // Clear
        ...
        // Log
        this.logger?.log(
            `[ApiAbort] #${pendingItem.sn} ${pendingItem.service.name}`,
        )
        // onAbort
        pendingItem.onAbort?.()
    }
    ...
    async sendData(
        ...
        if (data instanceof Uint8Array) {
            const preBuf = await this.flows.preSendBufferFlow.exec(
                {
                    buf: data,
                    sn: pendingApiItem?.sn,
                },
        ...
        if (this.options.debugBuf) {
            if (typeof data === 'string') {
                this.logger?.debug(
                    '[SendText]' +
                        (pendingApiItem ? ' #' + pendingApiItem.sn : '') +
                        ` length=${data.length}`,
                    data,
                )
    ...
    protected async _doCallApi<T extends string & keyof ServiceType['api']>(
        ...
        const promise = new Promise<ApiReturn<ServiceType['api'][T]['res']>>(
            async (rs) => {
                // GetService
                const service =
                    this.serviceMap.apiName2Service[apiName as string]
                if (!service) {
                    rs({
                        isSucc: false,
                ...
                // Encode
                const opEncode = TransportDataUtil.encodeApiReq(
                    this.tsbuffer,
                    service,
                    req,
                    this.dataType,
                    this.type === 'LONG' ? pendingItem.sn : undefined,
                )
                if (!opEncode.isSucc) {
                    rs({
                        isSucc: false,
    ...
    protected async _onRecvData(
        ...
        if (typeof data === 'string') {
            this.options.debugBuf &&
                this.logger?.debug('[RecvText]' + (sn ? ' #' + sn : ''), data)
        } else if (data instanceof Uint8Array) {
            this.options.debugBuf &&
                this.logger?.debug(
                    '[RecvBuf]' + (sn ? ' #' + sn : ''),
            ...
            // @deprecated
            // Pre Flow
            const pre = await this.flows.preRecvBufferFlow.exec(
                { buf: data, sn: sn },
                this.logger,
            )
            if (!pre) {
            ...
            this.options.debugBuf &&
                this.logger?.debug('[RecvJSON]' + (sn ? ' #' + sn : ''), data)
        }

        // Parse
        const opParsed = TransportDataUtil.parseServerOutout(
            this.tsbuffer,
            this.serviceMap,
            data,
            pendingApiItem?.service.id,
        )
        if (!opParsed.isSucc) {
            this.logger?.error('ParseServerOutputError: ' + opParsed.errMsg)
            if (data instanceof Uint8Array) {
                this.logger?.error(
                    'Please check the version of serviceProto between server and client',
                )
            }
            if (pendingApiItem) {
    ...
    protected async _waitApiReturn(
        ...
        return new Promise<ApiReturn<any>>((rs) => {
            ...
            if (timeout) {
                timer = setTimeout(() => {
                    timer = undefined
                    remove(this._pendingApis, (v) => v.sn === pendingItem.sn)
                    rs({
                        isSucc: false,
...
export type ClientMsgHandler<
    ServiceType extends BaseServiceType,
    MsgName extends keyof ServiceType['msg'],
