...
import { ApiService, TransportDataUtil } from '@ntsrpc/tsrpc-base-client'
import {
    ApiReturn,
    BaseServiceType,
    ServerOutputData,
...
> extends BaseCall<ServiceType> {
     ...
     */
    override readonly service!: ApiService
    /** Only exists in long connection, it is used to associate request and response.
     * It is created by the client, and the server would return the same value in `ApiReturn`.
     */
    readonly sn?: number
                ...
                new PrefixLogger({
                    ...
                    prefixs: [
                        `${chalk.cyan.underline(
                            `[Api:${options.service.name}]`,
                        )}${
                            options.sn !== undefined
                                ? chalk.gray(` SN=${options.sn}`)
                                : ''
                        }`,
                    ],
                }),
    ...
    static encodeApiReturn(
        ...
        if (type === 'buffer') {
            ...
            if (apiReturn.isSucc) {
                const op = tsbuffer.encode(apiReturn.res, service.resSchemaId)
                if (!op.isSucc) {
                    return op
                }
            ...
            const op = TransportDataUtil.tsbuffer.encode(
                serverOutputData,
                'ServerOutputData',
            )
            return op.isSucc
                ? { isSucc: true, output: op.buf }
                : { isSucc: false, errMsg: op.errMsg }
        } else {
            apiReturn = { ...apiReturn }
            if (apiReturn.isSucc) {
                const op = tsbuffer.encodeJSON(
                    apiReturn.res,
                    service.resSchemaId,
                )
                if (!op.isSucc) {
            ...
            const json =
                sn == undefined ? apiReturn : [service.name, apiReturn, sn]
            return {
                isSucc: true,
    ...
    protected async _prepareReturn(ret: ApiReturn<Res>): Promise<void> {
        ...
        // Pre Flow
        const preFlow = await this.server.flows.preApiReturnFlow.exec(
            ...
            if (opSend.canceledByFlow) {
                ...
                if (ret.isSucc || ret.err.type === TsrpcErrorType.ApiError) {
                    ...
                    this.server.onInternalServerError(
                        { message: opSend.errMsg, name: 'SendReturnErr' },
                        this,
                    )
                }
            }
    ...
    protected async _sendReturn(
        ret: ApiReturn<Res>,
    ): ReturnType<SendReturnMethod<Res>> {
        // Encode
        const opServerOutput = ApiCall.encodeApiReturn(
        ...
        const opSend = await this.conn.sendData(opServerOutput.output)
        if (!opSend.isSucc) {
            return opSend
        }
        return opSend
...
export type SendReturnMethod<Res> = (
    ret: ApiReturn<Res>,
