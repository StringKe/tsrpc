...
export class TSBufferValidator<Proto extends TSBufferProto = TSBufferProto> {
    ...
    constructor(proto: Proto, options?: Partial<TSBufferValidatorOptions>) {
        ...
        this.proto = this.options.cloneProto ? merge({}, proto) : proto
    ...
    validate(
        ...
        return this._validate(value, schema, {
            ...
            excessPropertyChecks:
                ...
                this.options.excessPropertyChecks,
            strictNullChecks:
    ...
    private _validate(
        ...
        return vRes as ValidateOutput
    }

    private _validateBooleanType(
        value: any,
        _schema: BooleanTypeSchema,
    ): ValidateResult {
    ...
    private _validateArrayType(
        ...
        for (let i = 0; i < value.length; ++i) {
            ...
            if (ValidateResultUtil.isNotSucc(elemValidateResult)) {
    ...
    private _canBeUndefined(schema: TSBufferSchema): boolean {
        ...
        return (
            schema.type === SchemaType.Literal && schema.literal === undefined
        )
    }
    ...
    private _canBeNull(schema: TSBufferSchema): boolean {
        ...
        return schema.type === SchemaType.Literal && schema.literal === null
    }
    ...
    private _validateAnyType(_value: any): ValidateResult {
        return ValidateResultUtil.succ
    }
        ...
        if (schema.properties) {
            for (const property of schema.properties) {
                ...
                if (
                    ...
                    const isOptional =
                        canBeUndefined ||
                        (!options.strictNullChecks && canBeNull)
                ...
                if (ValidateResultUtil.isNotSucc(vRes)) {
                    return ValidateResultUtil.innerError(
                        property.name,
                        value[property.name],
                        property.type,
    ...
    private _validateUnionType(
        ...
        options.unionProperties =
            options.unionProperties ||
            this.protoHelper.getUnionProperties(schema)
    ...
    private _validateNonNullableType(
        ...
        return this._validate(value, schema.target, options) as ValidateResult
    }
