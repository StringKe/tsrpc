...
export class AstParser {
    ...
    getFlattenNodes(
        ...
        node.forEachChild((v) => {
                ...
                output[v.name.text] = {
                    node:
                        v.kind === ts.SyntaxKind.TypeAliasDeclaration
                            ...
                            : v,
                    comment: (v as any).jsDoc?.[0]?.comment, // export default的情况，本体作为不isExport，取而代之生成一个名为default的TypeReference来export
            ...
            // namespace
                ...
                ts.isModuleDeclaration(v) &&
                v.flags & ts.NodeFlags.Namespace
            ) {
                if (v.body && v.body.kind === ts.SyntaxKind.ModuleBlock) {
                    ...
                    for (const item of Object.entries(children)) {
                        // 临时存储内部export
                        namespaceExports[v.name.text][item[0]] =
                            item[1].isExport
        ...
        if (
            output['default'] &&
            ts.isTypeReferenceNode(output['default'].node)
        ) {
            const typeName = this._typeNameToString(
                output['default'].node.typeName,
            )
    ...
    protected _node2schema(
        ...
        // ArrayType: Array<T>
        if (
            this._isLocalReference(node, imports, 'Array') &&
            node.typeArguments
        ) {
            return {
                type: SchemaType.Array,
                ...
                members: node.members.map((v, i) => {
                    if (v.initializer) {
                        ...
                        // 负数
                            ...
                            initializer =
                                parseFloat(v.initializer.operand.getText()) * -1
                            return {
                                id: i,
                                name: v.name.getText(),
                                value: initializer++,
            ...
            node.members.forEach((member, i) => {
                ...
                if (ts.isPropertySignature(member)) {
                    if (ts.isComputedPropertyName(member.name)) {
                        throw new Error(
                            'ComputedPropertyName is not supported at now',
                        )
                    }
                    if (!member.type) {
                        throw new Error(
                            `Field must have a type: ${member.name.text}`,
                        )
                    }
                ...
                // indexSignature
                    ...
                    if (!member.type || !member.parameters[0].type) {
                        throw new Error(
                            'Error index signature: ' + member.getText(),
                        )
                    }
        ...
        if (ts.isIndexedAccessTypeNode(node)) {
            ...
            if (ts.isLiteralTypeNode(node.indexType)) {
                ...
                if (
                    ...
                    throw new Error(
                        `Error indexType literal: ${node.getText()}`,
                    )
                }
            ...
            // A['a' | 'b']
                ...
                throw new Error(
                    `Error IndexedAccessType indexType: ${node.getText()}`,
                )
            }
        }
        ...
        if (this._isLocalReference(node, imports, ['Pick', 'Omit'])) {
            ...
            if (ts.isTypeReferenceNode(node)) {
                ...
                throw new Error(
                    `Invalid ts.Node kind for Pick/Omit: ${node['kind']}`,
                )
            }
        ...
        if (ts.isExpressionWithTypeArguments(node)) {
            return this._getReferenceTypeSchema(
    ...
    private _isLocalReference(
        ...
        if (ts.isTypeReferenceNode(node)) {
            ...
            ref = this._getReferenceTypeSchema(
                node.expression.getText(),
                imports,
            )
        } else {
    ...
    private _getPreKey(
        ...
        if (
            ...
            throw new Error(
                'Illeagle Pick keys: ' + JSON.stringify(schema, null, 2),
            )
        }
    }
...
export interface ScriptImports {
    ...
    [asName: string]: {
        path: string // import { A as B } A为targetName
        targetName: string
    }
}
