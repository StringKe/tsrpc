    ...
    private _read(schema: TSBufferSchema): unknown {
        switch (schema.type) {
            ...
            case SchemaType.Tuple: {
                ...
                const maxIndex = last(maskIndices) as number
                for (
            ...
            case SchemaType.Enum:
                ...
                if (!enumItem) {
                    throw new Error(
                        `Invalid enum encoding: unexpected id ${enumId}`,
                    )
                }
            ...
            case SchemaType.Custom:
                if (!schema.decode) {
                    throw new Error(
                        'Missing decode method for CustomTypeSchema',
                    )
                }
    ...
    private _readInterface(schema: InterfaceTypeSchema): unknown {
        ...
        for (let i = 0; i < blockIdNum; ++i) {
            ...
            // extend block
                ...
                const extend =
                    schema.extends &&
                    schema.extends.find((v) => v.id === extendId)
                if (extend) {
                    this._skipIdLengthPrefix(
            ...
            // property
                ...
                if (property) {
                    this._skipIdLengthPrefix(
                        this._validator.protoHelper.parseReference(
                            property.type,
                        ),
                    )
        ...
        for (const property of flatSchema.properties) {
            if (Object.prototype.hasOwnProperty.call(output, property.name)) {
                continue
            }
