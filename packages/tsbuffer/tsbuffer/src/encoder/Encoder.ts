...
import { InterfaceTypeSchema, IntersectionTypeSchema, NumberTypeSchema, OmitTypeSchema, OverwriteTypeSchema, PartialTypeSchema, PickTypeSchema, SchemaType, TSBufferSchema, TypeReference, UnionTypeSchema } from '@ntsrpc/tsbuffer-schema';
...
export class Encoder {
        ...
        switch (schema.type) {
            ...
            case SchemaType.Interface: {
                ...
                for (const key in value) {
                    const property = flatSchema.properties.find(
                        (v) => v.name === key,
                    )
                    if (property) {
            ...
            case SchemaType.Overwrite:
                const parsed =
                    this._validator.protoHelper.parseMappedType(schema)
    ...
    private _write(
        ...
        switch (schema.type) {
            ...
            case SchemaType.Custom:
                if (!schema.encode) {
                    throw new Error(
                        'Missing encode method for CustomTypeSchema',
                    )
                }
    ...
    private _processIdWithLengthType(
        ...
        if (lengthInfo.needLengthPrefix) {
            const payloadByteLength = sumBy(
                this._writer.ops.filter((v, i) => i > idPos),
                (value) => value.length,
            )
            this._writer.ops.splice(
